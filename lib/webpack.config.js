...
const { pageList, beforeCreate } = require("./utils");
...
if (isProd) {
  webpackConfig.plugins.push(
    ...
    new FileManagerPlugin({
      onStart: {
        delete: [path.join(process.cwd(), "docs/")]
      },
      onEnd: {
        copy: glob.sync(path.join(process.cwd(), 'static/*')).map(filePath => {
          const fileName = path.basename(filePath)
          return {
            source: filePath,
            destination: path.join(process.cwd(), "docs/static/", fileName)
          }
        })
      }
    })
