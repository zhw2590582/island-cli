...
module.exports = function(git, showHelp) {
  ...
  function downloanTheme() {
    ...
    downloadGit(gitUrl, themeDist).then(() => {
      logger.success(`Now, you need to install dependencies for the new theme by: cd src/theme/${themeName} && npm install. Then, you can go back and change config.js file to switch the theme`);
    });
  }
};
