...
exports.updateMeta = function updateMeta(name, creat = true) {
  ...
  if (metaData.poster) {
    metaData.poster = metaData.poster.charAt(0) === '/' ? metaData.poster.slice(1) : metaData.poster;
...
exports.beforeCreate = function beforeCreate(config) {
  config.web.page = {};
  glob.sync(path.join(process.cwd(), 'src/data/pages/', '*.md')).forEach(item => {
    ...
    config.web.page[filename] = htmlData;
  });
  return config;
}