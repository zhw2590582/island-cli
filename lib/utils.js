...
exports.updateMeta = function updateMeta(name, creat = true) {
    const db = exports.readDb();
    const newMeta = importFresh(path.join(process.cwd(), 'src/data/posts', `${name}/meta.js`));
    const currentDate = dayjs().format('YYYY-MM-DD HH:mm:ss');
    const postData = fs.readFileSync(path.join(process.cwd(), 'src/data/posts', `${name}/post.md`), 'utf-8');
